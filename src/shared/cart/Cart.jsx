    ...
    >
      ...
      >
        {cartItems?.length > 0 ?
          ...
          <Link to={"/products/all"} className="bg-blue-500 text-white hover:bg-neutral-800 rounded-lg py-2 px-5">Go To Shop</Link>
          </div>
      ...
      <div className="mt-4 grid grid-cols-4 gap-4 text-center">
        <Link to={"/cart"} className="col-span-2" onClick={() => setIsShow(false)}>
        <button className="  text-sm border border-black rounded-md px-5 py-2 w-full">
          View Cart
        </button>
        </Link>

        <Link to={"/checkout"} className="col-span-2" onClick={() => setIsShow(false)}>
        <button
          type="button"
