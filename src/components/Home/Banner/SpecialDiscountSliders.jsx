...
import useGetAllSpecialDiscountBanner from "../../../Hook/GetPublicDataHook/useGetAllSpecialDiscountBanner";
...
const CustomCarousel = ({ slides, isHomeArea = true }) => {
  ...
  return (
    <div
      className={`relativew-full h-[180px] ${
...
const SpecialDiscountSliders = ({ isHomeArea = true }) => {
  const [loading, setLoading] = useState(false);
  const banners = useGetAllSpecialDiscountBanner({ setLoading });

  return loading ? (
    <SkeletonCarousel />
  ) : (
    <CustomCarousel slides={banners} isHomeArea={isHomeArea} />
  );
};

export default SpecialDiscountSliders;
