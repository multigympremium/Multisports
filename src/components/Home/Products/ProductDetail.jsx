...
const ProductDetail = ({targetId, isShowDetail}) => {
  ...
  const increaseQuantity = () => {
    setQuantity((prev) => {
      const existingProduct = cartItems.find((item) => item._id === targetId && item.color === color && item.size === size);
  ...
  console.log(cartItems.map(item => item?._id).includes(targetId), "cartItems.map(item => item?._id).includes(targetId)")
...
<InnerImageZoom src={selectedImage} zoomSrc={selectedImage}
  width={400}
  hasSpacer={true} style={{height: "200px"}} />
  
</div>
          <div className="flex mt-4 space-x-4 w-full overflow-auto h-[100px]">
          <div  onClick={() => setSelectedImage(`https://mgpwebaps.s3.eu-north-1.amazonaws.com/multi-sports/${product?.thumbnail}` )}>
...
height={200}
  ...
  className={`w-28 h-full cursor-pointer border-2 ${
    selectedImage === `https://mgpwebaps.s3.eu-north-1.amazonaws.com/multi-sports/${product?.thumbnail}` ? 'border-blue-500' : 'border-gray-300'
  }`}
...
/>
            ...
            {product?.gallery?.length > 0 && product?.gallery.map((image, index) => (
              <div key={index} onClick={() => setSelectedImage(`https://mgpwebaps.s3.eu-north-1.amazonaws.com/multi-sports/${image.image}` )}>
                ...
                height={200}
                  ...
                  className={`w-28 h-full cursor-pointer border-2 ${
                    selectedImage === `https://mgpwebaps.s3.eu-north-1.amazonaws.com/multi-sports/${image.image}` ? 'border-blue-500' : 'border-gray-300'
                  }`}
        ...
        <div>
          ...
          <p className="text-sm mb-4 flex items-center gap-2">Color:  {
          product?.productColorValue &&  product?.productColorValue.map((item, index)=> (

                <span className={`h-6 w-6 rounded inline-block ${color === item ? "border-2 border-black" : ""}`} onClick={()=> setColor(item)} key={index} style={{background: item}}></span>
          ))
          } 
          </p>
          <div className="mb-4">
            ...
            { product?.productSizeValue && product?.productSizeValue.map((item, index) => (
              <button className={`px-4 py-2 rounded-md border-gray-300 ${size === item ? "border-2"  : ""}`} key={index} onClick={()=> setSize(item)} >{item}</button>
            ))}
          ...
          <button
            className="px-2 py-1 bg-gray-300 rounded disabled:opacity-20"
            onClick={() => cartItems.map(item => item?._id).includes(targetId) ? increaseQuantity(product._id): addToCart(product, color, size)}
            disabled={product?.stock <= 0 && color === "" || size === ""}
          >
            +
          </button>
          ...
          <div className="flex space-x-4">
            <button className="bg-gray-700 text-white px-6 py-3 rounded-md disabled:opacity-20" onClick={()=> addToCart(product, color, size)}  disabled={product?.stock <= 0 && color === "" || size === ""} >Add to Cart</button>
            <button className="bg-blue-700 text-white px-6 py-3 rounded-md disabled:opacity-20" disabled={product?.stock <= 0 && color === "" || size === "" || !cartItems.map(item => item?._id).includes(targetId) } onClick={()=> navigate(`/checkout`)}>Buy Now</button>
            <button className="bg-yellow-700 text-white px-6 py-3 rounded-md disabled:opacity-20" disabled={!cartItems.map(item => item?._id).includes(targetId) } onClick={()=>{ removeFromCart(targetId , color, size); setQuantity(0)}}>Remove from Cart</button>
          </div>
        </div>
      </div>
