...
import { AiOutlinePrinter, AiOutlineFile, AiOutlineDelete } from 'react-icons/ai';
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const res = await axiosSecure.delete(`/invoice/delete/${id}?branch=${data.branch}`);
        ...
        <div className="absolute top-2 right-2 flex space-x-2">
          ...
          <span
            className={`px-3 py-1 rounded-full text-white text-xs ${
              isActive(item?.end_date) === "Expired" ? "bg-red-600" : "bg-green-600"
            }`}
          >
          ...
          <span className="px-3 py-1 rounded-full text-gray-700 text-xs font-semibold bg-gray-100 shadow-sm">
            Branch: {item?.branch === "shia" ? "Shia Mosjid Branch" : item?.branch === "lalmatia" ? "Lalmatia Branch" : item?.branch}
          </span>
        </div>
            ...
            <td className="col-span-1 px-6 py-4">
              <div className="flex flex-col w-full h-full">
                <h4 className="text-sm font-semibold text-gray-700 mb-1">Start On</h4>
            ...
            <td className="text-center col-span-1 px-6 py-4">
              <div className="flex flex-col w-full h-full gap-1">
                <h4 className="text-sm font-semibold text-gray-700">Expire On</h4>
                <h5 className="text-sm text-gray-600">{item?.end_date}</h5>
              </div>
            </td>
            <td className="text-right col-span-1 px-6 py-4">
              <h4 className="text-sm font-semibold text-gray-700">Receipt No</h4>
              <h5 className="text-gray-700">{item?.receipt_no}</h5>
            </td>
            <td className="text-right col-span-1 px-6 py-4">
              <h4 className="text-sm font-semibold text-gray-700">Admission Fee</h4>
              <h5 className="text-gray-700">{item?.admissionFee}</h5>
            </td>
            <td className="text-right col-span-1 px-6 py-4">
              <h4 className="text-sm font-semibold text-gray-700">Package Fee</h4>
            ...
            <td className="text-right col-span-1 px-6 py-4">
              ...
              <h5 className="text-gray-800 font-semibold">
                {parseInt(item?.packageFee) + parseInt(item?.admissionFee) - parseInt(item?.discount)}
              </h5>
            </td>
            <td className="text-right col-span-1 px-6 py-4">
  <div className="flex items-center gap-3">
    ...
    >
      <AiOutlineDelete className="text-xl" />
        ...
        <h3 className="text-center py-6 text-2xl font-bold text-gray-600">No Data</h3>
      )}
    </div>
  );
}
